...
import 'package:video_player_web_dash/dash.dart';
import 'package:video_player_web_dash/no_script_tag_exception.dart';

import 'package:video_player_web_dash/src/duration_utils.dart';
...
/// Wraps a [html.VideoElement] so its API complies with what is expected by
/// the plugin.
class VideoPlayer {
  ...
  DashMediaPlayer? _dash;

  void Function(DashErrorEvent) get onError => allowInterop((DashErrorEvent e) {
        _eventController.addError(
          PlatformException(
            code: _kErrorValueToErrorName[2]!,
            message: e.type,
            details: typeofEquals<String>(e.error, 'string')
                ? e.error as String
                : (e.error as DashMediaPlayerErrorEvent).message,
          ),
        );
      });
  ...
  ///
  /// This method sets the required DOM attributes so videos
  /// can [play] programmatically,
  ...
  Future<void> initialize() async {
    _videoElement
      ...
      // Set autoplay to false since most browsers won't autoplay a
      // video unless it is muted
      ..setAttribute('autoplay', 'false');

    if (await _shouldUseDashLibrary()) {
      try {
        ...
        _dash!.extend(
          'RequestModifier',
          allowInterop(() {
            return DashExtendConfig(
              modifyRequestHeader: allowInterop(
                (html.HttpRequest xhr, _) {
                  if (headers.isEmpty) {
                    return xhr;
                  }
        ...
        _dash!.initialize(_videoElement, uri, false);
    ...
    // The error event fires when some form of error occurs while attempting
    // to load or perform the media.
  ...
  // Sends an [VideoEventType.initialized] [VideoEvent] with info
  // about the wrapped video.
  ...
  // Broadcasts the [html.VideoElement.buffered] status
  ...
  Future<bool> _shouldUseDashLibrary() async {
    if (!_isDashSupported) {
      return false;
    }
